---
title: 1.1.2: ansible-become(sudo)
...
#### 1) first create a common user and switch to it
``` bash
...
useradd classmate
```
> 实际上控制端不需要有classmate这个用户而使用root也可以，但是为了模仿线上环境，我们同样创建一个classmate用户
